...
/**
 * RSQL Spec Builder for handling generic node operations(AND, OR)
 * 
 * @author sulaimat
 *
 * @param <T>
...
public class GenericRsqlSpecBuilder<T> {
	...
	public Specification<T> createSpecification(final LogicalNode logicalNode) {
		...
		List<Specification<T>> specs = logicalNode.getChildren().stream().map(node -> createSpecification(node))
